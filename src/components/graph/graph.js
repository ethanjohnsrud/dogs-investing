...
/*
  ...
  Three Modes: Holdings, Performance, Market
...
const Graph = () => {
    ...
    const [dataType, setDataType] = useState('performance'); //Options: holding, performance, market
    ...
    /*
        ...
        axios.get(`https://api.cryptowat.ch/markets/:exchange/:pair/ohlc/`,{headers: {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json', 'X-CW-API-Key': '0LPHZD14RDU1JCXCYO6T'}, params: {'before': END_TIME, 'after': START_TIME, 'periods': INTERVAL }})
            .then((response) => { console.log('(DOGE && DOGE.length) API RESPONSE', response.data);});
...
const getBalance = (id, performance = false) => DOGS.find(d=>d.id==id).transactions
                    .reduce((p,t)=>(p + t.amount), 0) * ((performance && (DOGE && DOGE.length) ) ? DOGE[DOGE.length-1][4] : 1); //Convert to Todays USD Value
//Current Balance - Balance at Initial Investment
const getGrowth = (id, performance = false) => getBalance(id, performance) - ((performance && (DOGE && DOGE.length) )  
  ...
  : DOGS.filter(d=>SELECTED_ID.find(i=>i.id==d.id))
          .map(d => ({label: d.name, 
            data: DOGE.map((D) => ({primary: D[0], //Dates are in Seconds
                secondary: d.transactions.reduce((p,c)=> ((c.date/1000)<=(D[0])) ? p+=c.amount : p, 0)*(dataType === 'performance' ? D[4] : 1) //Dates in Milliseconds
          }))})));
  else setData()
    ...
    return (<div id='graph'>
      <div id='graph-chart' >
        <div id='graph-chart-header' >
          ...
          <label className={`graph-chart-mode${(dataType === 'performance') ? ' graph-chart-mode-selected' : ''}`} onClick={()=>setDataType('performance')}>YTD Performance [$]</label>
...
<div id='graph-analysis' >
          ...
          <label id='graph-analysis-header' >DOGECOIN {dataType.toUpperCase()}</label>
          {(dataType === 'holding') ?  
              <div id='graph-analysis-content' > {/* Holdings Portfolio Analysis  */}
                <label className='graph-label' >Current</label>
                    <label className='graph-detail' >{(DOGE && DOGE.length) ? (1.0/DOGE[DOGE.length-1][4]).toFixed(5) : 0}  &ETH; / USD</label>
                <label className='graph-label' >Total Holdings</label>
                  ...
                  <label className='graph-label' >Average Holdings</label>
                    <label className='graph-detail' >{(SELECTED_ID.length && DOGS.length) ? (SELECTED_ID.reduce((p,s)=>getBalance(s.id)+p, 0)/SELECTED_ID.length).toFixed(2) : 0} &ETH;</label>
          ...
          : (dataType === 'performance') ?  
              <div id='graph-analysis-content' > {/* Performance Portfolio Performance Analysis  */}
                <label className='graph-label' >Current</label>
